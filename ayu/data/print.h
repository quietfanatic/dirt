...
enum class PrintOptions {
    ...
    Pretty = 1 << 1,
     ...
     // may produce non-conforming output.  It does not currently verify that
     // strings are valid UTF-8; if they aren't, some JSON readers will reject
     // the JSON.
    Json = 1 << 2,
     // For validation
...
DECLARE_ENUM_BITWISE_OPERATORS(PrintOptions)
 ...
 // Converts a tree to an AYU string.  May be significantly overallocated
 // because it assumes you won't keep the string around for long.  If you're
 // keeping lots of these around for a long time, you may want to call
