...
struct SharableBuffer {
    ...
    static constexpr usize plenty_for_size (usize size) {
         ...
         // There isn't a whole lot of science to these choices, besides that
         // AYU tends to have a lot of arrays of size 2*16 bytes.
    ...
    T* allocate (usize size) {
         // Use u64 instead of usize because on 32-bit platforms we need to make
         // sure we don't overflow usize.
